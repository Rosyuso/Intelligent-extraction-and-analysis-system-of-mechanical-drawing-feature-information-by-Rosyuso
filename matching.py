...
def get_digits(s):
    ...
    # 返回的第一个值不带'∅'，如35，第二个值是配合，如k6

    # if s[0] == '∅' and 'X' or 'k' not in s: # 先用直径试一下
    # if s[0] == '∅':  # 先用直径试一下
    #     for i in ['f','g','h','js','k','m','n']:
    #         if i in s:
    #             d.append(s.split(i)[0].split('∅')[1])  ## 特例需修改
    #         else:
    #             for j in s:
...
def text_match_coor(coor, rect, text, shaft_length):
    ...
    axis_r = int(np.mean(center_r, axis=0)[1])
...
rect_width = [float(format(i / ratio, '.1f')) for i in rect_width]
rect_height = [float(format(i / ratio, '.1f')) for i in rect_height]

rect_info = list(zip(rect_width, rect_height, rect_center))
# 按照矩形中心横坐标从左到右排列
...
# 其中前两项为矩形的宽和高，，第三项为中心坐标
# 水平轴线
multi_blcok, single_block = [], []  # 用于收集每个矩形的尺寸信息
for i in range(len(rect_width)):  # i是矩形索引
    for j in range(len(coor_center)):  # j是尺寸索引
        ...
        elif abs(axis_r - coor_center[j][1]) > 0.5 * rect_info[i][1]:  # 远离中心轴线的尺寸，可以看作横向长度尺寸
            if coor_center[j] == 'taken': continue
            ...
            if text[j].isdigit() and \
                ...
                for k in range(i + 1, len(rect_info)):
                    ...
                    if int(text[j]) - length <= aseb_thre_near:  # 说明已经加到了尺寸范围附近
                        # 从i到k即为该尺寸覆盖的矩形块
                        multi_blcok.append([i, k, length, int(text[j])])  # 结果存到列表里
                        print('匹配到结果--->   第{}到{}个矩形块的长度为:{}，\t尺寸:{}'.format(i + 1, k + 1, length, int(text[j])))

            # 除了位置固定的长宽以外，其余的包括退刀槽，螺纹等位置随机的尺寸

            # 退刀槽尺寸特点： 形如3X∅12，或者无∅：6X3, 或者只有一个数字：

            # 退刀槽考虑从尺寸角度进行匹配，先确定槽宽和槽深，然后寻找合适的矩形？？
            # 螺纹同理
            # 待添加——————————————————————————————————————————————————————————————

print('multi_block->', multi_blcok)

print('single_block->', single_block)
